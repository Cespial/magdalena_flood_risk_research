"""
Shared utility functions for the Magdalena Flood Risk Assessment project.
...
# Expected Magdalena area in km2 (DANE official)
MAGDALENA_AREA_KM2 = 23_188.0
MAGDALENA_AREA_TOLERANCE = 0.05  # 5% tolerance for area validation
...
def load_magdalena_boundary(source: str = "gadm") -> gpd.GeoDataFrame:
    """
    Load the Magdalena department boundary polygon.
    ...
    """
    paths = {
        "gadm": BOUNDARIES_DIR / "magdalena_department_boundary_GADM41.geojson",
        "geoboundaries": BOUNDARIES_DIR / "magdalena_department_boundary_geoBoundaries.geojson",
        "naturalearth": BOUNDARIES_DIR / "magdalena_department_naturalearth.geojson",
    }
    path = paths.get(source)
    if path is None or not path.exists():
        raise FileNotFoundError(
            f"Magdalena boundary not found for source='{source}'. "
...
def load_municipalities(source: str = "gadm") -> gpd.GeoDataFrame:
    """
    Load Magdalena municipality polygons (30 municipalities).
    ...
    """
    paths = {
        "gadm": BOUNDARIES_DIR / "magdalena_municipalities_30_GADM41.geojson",
        "geoboundaries": BOUNDARIES_DIR / "magdalena_municipalities_geoBoundaries_simplified.geojson",
    }
    path = paths.get(source)
    if path is None or not path.exists():
        raise FileNotFoundError(
...
def load_subregions() -> gpd.GeoDataFrame:
    """Load the 5 official subregions of Magdalena."""
    path = BOUNDARIES_DIR / "magdalena_5_subregions.geojson"
    if not path.exists():
...
def load_river_basins(level: int = 5) -> gpd.GeoDataFrame:
    """
    Load HydroSHEDS river basins clipped to Magdalena.
    ...
    """
    path = BOUNDARIES_DIR / f"magdalena_river_basins_HydroSHEDS_L{level}.geojson"
    if not path.exists():
...
def validate_magdalena_area(gdf: gpd.GeoDataFrame, tolerance: float = 0.05) -> bool:
    """
    Check whether the total area of gdf is close to the expected
    Magdalena area (23,188 km2) within the given fractional tolerance.
    """
    area = compute_area_km2(gdf)
    expected = MAGDALENA_AREA_KM2
