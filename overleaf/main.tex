...
ESA WorldCover v200 & ESA & \SI{10}{\metre} & 2021 & Land cover \citep{Zanaga2022} \\
WorldPop v2020 & U. Southampton & \SI{100}{\metre} & 2020 & Population density \citep{Tatem2017} \\
HydroSHEDS & WWF/USGS & Vector & 2023 & River network \citep{Lehner2008} \\
GADM v4.1 & U.C. Davis & Vector & 2024 & Admin. boundaries \\
\bottomrule
\end{tabular}
\end{table}

Sentinel-1 C-band SAR GRD scenes were ingested (IW mode, VV+VH polarization) using \textit{both ascending and descending} orbit passes. The dual-orbit strategy is essential for Magdalena: the Sierra Nevada de Santa Marta creates extensive radar shadow and layover zones in descending pass geometry, which ascending passes partially compensate. The Sentinel-1B failure (December 2021 to April 2024) reduced revisit to 12 days; Sentinel-1C deployment in December 2024 restored 6-day coverage. The JRC Global Surface Water dataset \citep{Pekel2016} provided 38-year water dynamics for validation and as an input feature. Terrain variables (elevation, slope, aspect, curvature, Topographic Wetness Index [TWI], and Stream Power Index [$\text{SPI}_{\text{t}}$]) were derived from SRTM \SI{30}{\metre} \citep{Farr2007}. HAND was computed from MERIT Hydro \SI{90}{\metre} \citep{Yamazaki2019} and resampled to \SI{30}{\metre} via bilinear interpolation within GEE. While resampling does not add spatial information beyond the native \SI{90}{\metre}, it enables pixel-aligned stacking with the \SI{30}{\metre} feature grid; the effective resolution of the HAND predictor remains \SI{90}{\metre}, and its vertical accuracy ($\pm$\SI{5}{\metre}, inherited from SRTM) should be considered when interpreting threshold-based results (e.g., HAND $< \SI{5}{\metre}$), particularly in the low-relief floodplains of the Depresi\'on Momposina and R\'io Magdalena corridor.
...
The SAR workflow (Figure~\ref{fig:methodology}) transforms Sentinel-1 backscatter into monthly binary water maps through: (1) speckle reduction via focal median filter (\SI{50}{\metre} kernel) with slope masking ($> 30^{\circ}$); (2) adaptive Otsu thresholding \citep{Otsu1979} on the VV histogram; (3) water mask refinement with \SI{1}{\hectare} minimum mapping unit and permanent water flagging (JRC occurrence $\geq 75\%$); and (4) monthly maximum-extent compositing. This change-detection approach builds on multi-temporal SAR flood mapping principles \citep{Clement2018}. Ascending and descending orbit water maps are merged via logical union to maximize spatial coverage. Special care was taken near the Ci\'enaga Grande de Santa Marta and the Caribbean coast, where tidal fluctuations and estuarine dynamics can produce non-fluvial water signals that must be distinguished from flood events.
...
\begin{equation}
    ...
    \label{eq:flood_freq}
\end{equation}
where $M$ is the total number of monthly composites, and classified into five categories: permanent ($>$75\%), very frequent (50--75\%), frequent (25--50\%), occasional (10--25\%), and rare (1--10\%). Validation against JRC occurrence used overall accuracy, Cohen's $\kappa$, F1-score, and commission/omission error rates computed from stratified random samples at \SI{30}{\metre}.
...
\multirow{7}{*}{Topographic} & Elevation, Slope, Aspect & SRTM \\
 & Plan curvature & SRTM \\
 & HAND (m) & MERIT Hydro \\
 & TWI, $\text{SPI}_{\text{t}}$ & MERIT Hydro + SRTM \\
\midrule
Hydrological & Dist. to drainage, Drainage density & HydroSHEDS \\
\midrule
Climatic & Mean/Max precip., Soil moisture & CHIRPS, ERA5 \\
\midrule
Land/Demog. & Land cover, NDVI, Pop. density, Accessibility$^*$ & WorldCover, S2, WorldPop, Oxford MAP \\
\midrule
Water hist. & JRC occurrence, SAR flood freq. & JRC, This study \\
\bottomrule
...
\begin{equation}
    ...
    \label{eq:fri}
\end{equation}
where $FF_{\text{mean},j}$ is expressed as a fraction (0--1), not a percentage. Sensitivity to the threshold $\tau$ was tested at 0.5, 0.6, and 0.7.

\subsection{Seasonal and ENSO Analysis}

Precipitation climatology was derived from CHIRPS Daily v2.0 \citep{Funk2015} aggregated over the study area for 2015--2025. Monthly water extent was stratified by ENSO phase using the Oceanic Ni\~no Index (ONI $> 0.5$\textdegree{}C for El Ni\~no, $< -0.5$\textdegree{}C for La Ni\~na, over $\geq$5 consecutive overlapping 3-month periods). Differences were tested via Kruskal-Wallis with Dunn's post-hoc correction. The Standardized Precipitation Index (SPI) \citep{McKee1993} was computed at 3-, 6-, and 12-month accumulation windows to identify wet and dry anomalies. Precipitation trends were assessed using the Mann-Kendall test with Sen's slope estimator. Land surface temperature (LST) trends were derived from MODIS MOD11A2 v061 (\SI{1}{\kilo\metre}, 8-day composite).
...
The Sentinel-1 processing yielded 132 monthly composites and 11 annual maximum extent maps across 2015--2025. Adaptive Otsu thresholds varied seasonally, confirming the necessity of adaptive thresholding over Magdalena's mixed terrain. Validation against JRC Global Surface Water across 12,509 stratified random points yielded a mean overall accuracy of 0.918, Cohen's $\kappa = 0.867$, and F1-score of 0.896 (Table~\ref{tab:sar_accuracy}). Performance was stable across years (OA range: 0.882--0.956), with slightly higher commission error in coastal pixels near the Ci\'enaga Grande.

\begin{table}[htbp]
\caption{Annual accuracy metrics for Sentinel-1 SAR water detection validated against JRC Global Surface Water (stratified random sampling).}
\label{tab:sar_accuracy}
\centering
\footnotesize
\begin{tabular}{lccccr}
\toprule
\textbf{Year} & \textbf{OA} & \textbf{$\kappa$} & \textbf{F1} & \textbf{Comm.\ Err.} & \textbf{$n$} \\
\midrule
2015 & 0.910 & 0.927 & 0.858 & 0.113 & 730 \\
2016 & 0.956 & 0.912 & 0.903 & 0.038 & 1,517 \\
2017 & 0.939 & 0.843 & 0.864 & 0.048 & 540 \\
2018 & 0.928 & 0.840 & 0.887 & 0.034 & 1,551 \\
2019 & 0.892 & 0.840 & 0.890 & 0.059 & 634 \\
2020 & 0.892 & 0.853 & 0.850 & 0.065 & 700 \\
2021 & 0.885 & 0.878 & 0.937 & 0.054 & 1,279 \\
2022 & 0.949 & 0.868 & 0.915 & 0.105 & 1,429 \\
2023 & 0.928 & 0.852 & 0.933 & 0.062 & 1,556 \\
2024 & 0.937 & 0.887 & 0.928 & 0.055 & 1,571 \\
2025 & 0.882 & 0.835 & 0.896 & 0.079 & 1,002 \\
\midrule
\textbf{Mean} & \textbf{0.918} & \textbf{0.867} & \textbf{0.896} & \textbf{0.065} & \textbf{12,509} \\
\bottomrule
\end{tabular}
\end{table}

The decadal flood frequency analysis (Figure~\ref{fig:flood_freq}) reveals three principal flooding regimes, spatially consistent with JRC long-term water occurrence (Figure~\ref{fig:jrc_hand}, left) and HAND values (Figure~\ref{fig:jrc_hand}, right): (1) permanent and semi-permanent inundation in the Ci\'enaga Grande de Santa Marta and associated wetlands; (2) seasonal fluvial flooding along the R\'io Magdalena corridor and the Depresi\'on Momposina; and (3) intermittent flooding in piedmont areas at the base of the Sierra Nevada de Santa Marta.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig04_flood_frequency.pdf}
\caption{SAR-derived flood frequency map (2015--2025) for the Department of Magdalena, classified into five categories from rare ($<$10\%) to permanent ($>$75\%).}
\label{fig:flood_freq}
\end{figure}
...
The weighted ensemble performance is summarized in Table~\ref{tab:ml_results}. The inter-fold AUC-ROC variation reflects the geographic heterogeneity of Magdalena, with stronger performance in the R\'io and Sur subregions (where strong HAND gradients maximize discrimination) and weaker performance in the Norte subregion (where the Ci\'enaga Grande's complex water dynamics reduce predictor contrast).
...
SHAP analysis identified HAND as the dominant predictor (29.3\% relative importance), followed by slope (19.6\%), JRC water occurrence (12.8\%), and elevation (12.0\%) (Figure~\ref{fig:roc_shap}, right). These four topographic and hydrological variables account for 73.7\% of total SHAP importance, underscoring the primacy of terrain geometry for flood susceptibility in Magdalena. HAND exhibits sharp nonlinearity: susceptibility is high for HAND $< \SI{5}{\metre}$ (covering extensive areas of the R\'io Magdalena floodplain and Depresi\'on Momposina), moderate at 5--\SI{15}{\metre}, and negligible above \SI{30}{\metre}---consistent with \citet{Nobre2011}. Distance to rivers (8.9\%) and TWI (8.9\%) constitute a second tier of predictors, while climatic and land cover variables contribute less ($<$5\% each), suggesting that in Magdalena's low-relief terrain, static topographic structure dominates over dynamic climate inputs.
...
The susceptibility map (Figure~\ref{fig:susceptibility}) classifies a substantial fraction of the department as high or very high susceptibility (Table~\ref{tab:suscept_area}), reflecting Magdalena's extensive lowland terrain. Three areas concentrate the highest values:
...
\subsubsection{Precipitation Climatology}

CHIRPS analysis reveals mean annual precipitation of \SI{1262}{\milli\metre} (2015--2025), with pronounced interannual variability ranging from \SI{851}{\milli\metre} (2015, a strong El Ni\~no year) to \SI{1956}{\milli\metre} (2022, during the triple-dip La Ni\~na). Precipitation is strongly seasonal: the SON quarter dominates with a mean of \SI{577}{\milli\metre} (43\% of the seasonal total), followed by JJA (\SI{422}{\milli\metre}, 31\%), MAM (\SI{291}{\milli\metre}, 22\%), and DJF (\SI{60}{\milli\metre}, 4\%). The Mann-Kendall test detected no statistically significant trend in annual precipitation ($\tau = 0.345$, $p = 0.161$, Sen's slope $= +40.5$~mm\,yr$^{-1}$) nor in any individual season over this 11-year record (Table~\ref{tab:precip_trends}).

\begin{table}[htbp]
\caption{Mann-Kendall trend analysis of precipitation (2015--2025).}
\label{tab:precip_trends}
\centering
\footnotesize
\begin{tabular}{lcccc}
\toprule
\textbf{Variable} & \textbf{Trend} & \textbf{$p$-value} & \textbf{$\tau$} & \textbf{Sen's slope (mm\,yr$^{-1}$)} \\
\midrule
Annual precipitation & No trend & 0.161 & 0.345 & +40.5 \\
DJF (dry season 1) & No trend & 0.087 & 0.418 & +4.7 \\
MAM (wet season 1) & No trend & 0.213 & 0.309 & +8.5 \\
JJA (dry season 2) & No trend & 0.213 & 0.309 & +16.6 \\
SON (wet season 2) & No trend & 0.533 & 0.164 & +4.5 \\
\bottomrule
\end{tabular}
\end{table}

Seven months exceeded the 95th percentile threshold (\SI{245}{\milli\metre\per month}), with October 2022 recording the maximum (\SI{425}{\milli\metre})---1.73$\times$ the threshold---during the peak of the triple-dip La Ni\~na. October concentrates the most extreme events (5 of 7 exceedances, 71\%), consistent with its role as the peak flood month.

Land surface temperature (MODIS LST) showed a statistically significant decreasing trend ($\tau = -0.60$, $p = 0.013$, Sen's slope $= -0.31$\textdegree{}C\,yr$^{-1}$), declining from \SI{33.0}{\degreeCelsius} (2015) to \SI{28.6}{\degreeCelsius} (2025). This cooling trend may partly reflect increased cloud cover and evapotranspiration associated with the wetter years in the latter half of the record.

\subsubsection{ENSO Stratification}

ENSO stratification (Table~\ref{tab:enso}) reveals La Ni\~na years with mean annual precipitation of \SI{1504}{\milli\metre} versus \SI{1268}{\milli\metre} during El Ni\~no---an 18.6\% increase. Neutral years averaged \SI{1323}{\milli\metre}. The Kruskal-Wallis test did not reach statistical significance at $\alpha = 0.05$ ($H = 3.48$, $p = 0.18$), likely due to the small sample size ($n = 11$ years), but the effect sizes are consistent with established ENSO--Caribbean precipitation linkages \citep{Poveda2001, Poveda2011}.

\begin{table}[htbp]
\caption{Precipitation and flood extent statistics by ENSO phase (2015--2025). El Ni\~no years: 2015, 2016, 2019, 2024; Neutral: 2017, 2018, 2023, 2025; La Ni\~na: 2020, 2021, 2022.}
\label{tab:enso}
\centering
\footnotesize
\begin{tabular}{lrrrrr}
\toprule
\textbf{ENSO phase} & \textbf{$n$} & \textbf{Mean precip. (mm)} & \textbf{Mean flood (km$^2$)} & \textbf{$\Delta$ precip.} & \textbf{$\Delta$ flood} \\
\midrule
El Ni\~no  & 4 & 1,268 $\pm$ 291 & 215 $\pm$ 57 & --- & --- \\
Neutral    & 4 & 1,323 $\pm$ 158 & 212 $\pm$ 26 & +4.3\% & $-$1.6\% \\
La Ni\~na  & 3 & 1,504 $\pm$ 399 & 269 $\pm$ 42 & +18.6\% & +25.0\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig12_climate_flood_correlation.pdf}
\caption{Climate--flood relationships (2015--2025). (\textbf{a})~Annual precipitation and mean flood extent by ENSO phase. (\textbf{b})~SPI-3 time series showing wet (blue) and dry (red) anomalies.}
\label{fig:climate_flood}
\end{figure}
...
Future priorities include: deep learning SAR segmentation (U-Net architectures), tidal correction for coastal flood discrimination, integration with CMIP6 downscaled precipitation for forward-looking risk assessment, coupling with sea-level rise projections for the Caribbean coast, and extension of the temporal record beyond 11 years to detect secular precipitation trends (currently non-significant at $p = 0.161$ but with a positive Sen's slope of +40.5~mm\,yr$^{-1}$).

\subsection{Reproducibility and Transferability}

A central design goal of this framework is full reproducibility. The entire pipeline---from SAR ingestion to municipality-level risk statistics---runs on Google Earth Engine for satellite processing and open-source Python libraries (scikit-learn, XGBoost, LightGBM, SHAP) for machine learning, requiring no proprietary software or local high-performance computing. All source code, configuration files, and trained models are publicly available under the MIT license. The framework has been successfully replicated across multiple Colombian departments by changing only the configuration file (study area extent, municipality list, and subregion definitions), demonstrating its scalability. This design aligns with the growing emphasis on reproducible geospatial science and open data principles in the flood risk community.
...
\textbf{Scientific contribution:} Processing Sentinel-1 scenes across \SI{23188}{\kilo\metre\squared} and 11 years using dual orbit passes, a weighted ensemble of RF, XGBoost, and LightGBM was evaluated under spatial cross-validation. HAND emerged as the dominant predictor (29.3\% SHAP importance), with a $<$\SI{5}{\metre} threshold capturing the physically meaningful fluvial envelope across Magdalena's extensive lowlands. La Ni\~na years amplify mean annual precipitation by 18.6\% relative to El Ni\~no years (\SI{1504}{\milli\metre} vs.\ \SI{1268}{\milli\metre}), with October concentrating 71\% of extreme precipitation events (5 of 7)---providing a precise temporal and climatic basis for ENSO-based early warning.
...
$\text{SPI}_{\text{t}}$ & Stream Power Index (topographic)\\
SPI & Standardized Precipitation Index (climatic)\\
