...
CHIRPS analysis reveals mean annual precipitation of \SI{1262}{\milli\metre} (2015--2025), with pronounced interannual variability ranging from \SI{851}{\milli\metre} (2015, a strong El Ni\~no year) to \SI{1956}{\milli\metre} (2022, during the triple-dip La Ni\~na). Precipitation is strongly seasonal: the SON quarter dominates with a mean of \SI{577}{\milli\metre} (46\% of the annual total), followed by JJA (\SI{422}{\milli\metre}, 33\%), MAM (\SI{291}{\milli\metre}, 23\%), and DJF (\SI{60}{\milli\metre}, 5\%). The Mann-Kendall test detected no statistically significant trend in annual precipitation ($\tau = 0.35$, $p = 0.16$, Sen's slope $= +41$~mm\,yr$^{-1}$) nor in any individual season over this 11-year record (Table~\ref{tab:precip_trends}).
...
Seven months exceeded the 95th percentile threshold (\SI{245}{\milli\metre\per month}), with October 2022 recording the maximum (\SI{425}{\milli\metre})---1.73$\times$ the threshold---during the peak of the triple-dip La Ni\~na. October concentrates the most extreme events (4 of 7 exceedances), consistent with its role as the peak flood month.
...
\begin{table}[htbp]
\caption{Precipitation and flood extent statistics by ENSO phase (2015--2025).}
\label{tab:enso}
\centering
\footnotesize
\begin{tabular}{lrrrr}
\toprule
\textbf{ENSO phase} & \textbf{$n$ years} & \textbf{Mean precip. (mm)} & \textbf{SD precip. (mm)} & \textbf{vs.\ El Ni\~no} \\
\midrule
El Ni\~no  & 4 & 1,268 & 291 & --- \\
Neutral    & 4 & 1,323 & 158 & +4.3\% \\
La Ni\~na  & 3 & 1,504 & 399 & +18.6\% \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Scientific contribution:} Processing Sentinel-1 scenes across \SI{23188}{\kilo\metre\squared} and 11 years using dual orbit passes, a weighted ensemble of RF, XGBoost, and LightGBM was evaluated under spatial cross-validation. HAND is expected to emerge as the dominant predictor, with a $<$\SI{5}{\metre} threshold capturing the physically meaningful fluvial envelope across Magdalena's extensive lowlands. La Ni\~na years amplify mean annual precipitation by 18.6\% relative to El Ni\~no years (\SI{1504}{\milli\metre} vs.\ \SI{1268}{\milli\metre}), with October concentrating 57\% of extreme precipitation events---providing a precise temporal and climatic basis for ENSO-based early warning.
...
SPI & Stream Power Index / Standardized Precipitation Index\\
