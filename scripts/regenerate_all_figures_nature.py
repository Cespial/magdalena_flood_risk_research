...
========================================
Nature-quality figures for Magdalena flood risk manuscript.
...
def load_boundary():
    return gpd.read_file(BOUNDARIES_DIR / "magdalena_department_boundary_GADM41.geojson")


def load_municipalities():
    return gpd.read_file(BOUNDARIES_DIR / "magdalena_municipalities_30_GADM41.geojson")


def load_subregions():
    return gpd.read_file(BOUNDARIES_DIR / "magdalena_5_subregions.geojson")
...
def fig01_study_area():
    ...
    magdalena = load_boundary()
    subregions = load_subregions()
    munis = load_municipalities()

    magdalena_p = magdalena.to_crs(CRS_COLOMBIA)
    ...
    if has_colombia:
        ...
        magdalena_p.plot(ax=ax, color='#d4a574', edgecolor='#333333', linewidth=0.5)
    else:
        magdalena_p.plot(ax=ax, color='#d4a574', edgecolor='#333333', linewidth=0.5)
    ...
    magdalena_p.boundary.plot(ax=ax, color='#333333', linewidth=0.6)
...
def fig02_sar_water_detection():
    ...
    for ax, img, label, title in panels:
        ...
        magdalena.boundary.plot(ax=ax, color='#444444', linewidth=0.3)
