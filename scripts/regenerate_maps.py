...
as PNG arrays, then overlays on proper Magdalena boundaries (WGS84).
...
from utils import (
    load_magdalena_boundary, load_municipalities,
...
def get_magdalena_region():
    """Get Magdalena geometry from FAO GAUL in GEE."""
    magdalena = (
        ee.FeatureCollection('FAO/GAUL/2015/level1')
        .filter(ee.Filter.eq('ADM0_NAME', 'Colombia'))
        .filter(ee.Filter.eq('ADM1_NAME', 'Magdalena'))
    )
    return magdalena.geometry().dissolve()


def get_magdalena_bbox(region):
    """Get the bounding box of Magdalena as [west, south, east, north]."""
...
def fig02_sar_water_detection():
    ...
    Rio Magdalena region (Sitionuevo-Remolino-Pivijay) — most flood-prone area.
    Pre-flood: Jul-Aug 2024 (dry), During-flood: Oct-Nov 2024 (wet).
    """
    print("Generating Figure 2: SAR water detection (REAL DATA)...")
    set_publication_style()

    # Focus on Rio Magdalena / Nechí / lower Cauca River — most flood-prone
    ...
    fig.suptitle(
        "Sentinel-1 SAR Flood Detection — Zona Rio, Magdalena",
...
def fig03_jrc_water_occurrence():
    """Download real JRC water occurrence and overlay on Magdalena boundary (WGS84)."""
    print("Generating Figure 3: JRC water occurrence (REAL DATA)...")
    set_publication_style()

    region = get_magdalena_region()
    bbox = get_magdalena_bbox(region)
    magdalena_gdf = load_magdalena_boundary("gadm")  # WGS84
    ...
    magdalena_gdf.boundary.plot(ax=ax, color="black", linewidth=1.2)

    # Set axis limits to Magdalena extent with small padding
    pad = 0.05
    ...
    ax.set_title("JRC Global Surface Water Occurrence, Magdalena", fontsize=10)
    add_north_arrow(ax)
