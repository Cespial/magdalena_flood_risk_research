...
=====================
Quality control and validation for the Magdalena Flood Risk Assessment.

Checks:
  1. All expected outputs exist and are non-empty
  2. Area calculations (Magdalena total ~ 23,188 km2)
...
from utils import (
    ...
    compute_area_km2, validate_magdalena_area,
    OUTPUTS_DIR, TABLES_DIR, FIGURES_DIR, OVERLEAF_FIGURES, OVERLEAF_TABLES,
    MAGDALENA_AREA_KM2, MAGDALENA_AREA_TOLERANCE,
...
def validate_areas() -> List[QCResult]:
    """
    Validate that area calculations for Magdalena boundaries are
    consistent with the expected 23,188 km2 (DANE official area).

    Tests:
    - GADM department boundary area
    - Sum of 30 municipality areas
    - Sum of 5 subregion areas
    ...
    for source in ["gadm", "geoboundaries", "naturalearth"]:
        try:
            boundary = load_magdalena_boundary(source)
            area_km2 = compute_area_km2(boundary)
            within_tol = abs(area_km2 - MAGDALENA_AREA_KM2) / MAGDALENA_AREA_KM2 <= MAGDALENA_AREA_TOLERANCE
            diff_pct = (area_km2 - MAGDALENA_AREA_KM2) / MAGDALENA_AREA_KM2 * 100
            ...
            r = QCResult(
                ...
                passed=within_tol,
                message=(
                    f"Magdalena area ({source}): {area_km2:,.1f} km2 "
                    f"(expected ~{MAGDALENA_AREA_KM2:,.0f} km2, "
                    f"diff = {diff_pct:+.2f}%)"
                ),
                details=f"Tolerance: {MAGDALENA_AREA_TOLERANCE*100:.0f}%",
    ...
    try:
        ...
        within_tol = abs(mun_total - MAGDALENA_AREA_KM2) / MAGDALENA_AREA_KM2 <= MAGDALENA_AREA_TOLERANCE
        diff_pct = (mun_total - MAGDALENA_AREA_KM2) / MAGDALENA_AREA_KM2 * 100
        ...
        r = QCResult(
            ...
            passed=within_tol,
            message=(
                f"Sum of 30 municipalities: {mun_total:,.1f} km2 "
                f"(diff = {diff_pct:+.2f}%)"
            ),
            severity="ERROR" if not within_tol else "INFO",
        )
    ...
    try:
        ...
        within_tol = abs(sub_total - MAGDALENA_AREA_KM2) / MAGDALENA_AREA_KM2 <= MAGDALENA_AREA_TOLERANCE
        diff_pct = (sub_total - MAGDALENA_AREA_KM2) / MAGDALENA_AREA_KM2 * 100
        ...
        r = QCResult(
            ...
            passed=within_tol,
            message=(
                f"Sum of 5 subregions: {sub_total:,.1f} km2 "
                f"(diff = {diff_pct:+.2f}%)"
            ),
...
def verify_municipal_stats() -> List[QCResult]:
    ...
    - All 30 municipalities are accounted for
    ...
    try:
        ...
        r = QCResult(
            ...
            category="municipal_stats",
            passed=(n_sub == 5),
            message=f"Subregion count: {n_sub} (expected 5)",
            severity="ERROR" if n_sub != 5 else "INFO",
        )
        results.append(r)
        _add_result(r)
    except FileNotFoundError:
    ...
    if exposure_path.exists():
        ...
        if "Total Population" in exp_df.columns and "Exposed Population" in exp_df.columns:
            data_rows = exp_df[exp_df["Subregion"] != "TOTAL (Magdalena)"]
...
def generate_qc_report() -> pathlib.Path:
    ...
    lines.append(f"## Magdalena Flood Risk Assessment")
    lines.append("")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("QUALITY CONTROL - MAGDALENA FLOOD RISK ASSESSMENT")
    logger.info("=" * 70)
