...
======================
Climate analysis for flood context in Magdalena, Colombia.
...
# ---------------------------------------------------------------------------
# GEE helper: get Magdalena geometry
# ---------------------------------------------------------------------------

def get_magdalena_geometry() -> ee.Geometry:
    """Return the Magdalena department geometry from FAO GAUL."""
    admin = ee.FeatureCollection(ADMIN_DATASET)
    magdalena = admin.filter(ee.Filter.eq("ADM1_NAME", DEPARTMENT_NAME))
    return magdalena.geometry()
...
def compute_precipitation_trends() -> pd.DataFrame:
    ...
    the spatially averaged total precipitation over Magdalena.  Then applies
...
def compute_temperature_trends() -> pd.DataFrame:
    ...
    MODIS MOD11A2 (8-day composites) over Magdalena.
...
def compute_spi(
    ...
    """
    Compute the Standardized Precipitation Index (SPI) for Magdalena
...
def extreme_precipitation_events(
    ...
    threshold computed from the full CHIRPS record over Magdalena.
...
def precipitation_flood_correlation(
    ...
    SAR-detected flood extent over Magdalena.
...
def _plot_precipitation_trends(precip_df: pd.DataFrame) -> None:
    ...
    ax.set_title("(a) Annual precipitation trend, Magdalena (CHIRPS)")
...
def _plot_spi_timeseries(spi_df: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("SPI-3")
    ax.set_xlabel("Date")
    ax.set_title("Standardized Precipitation Index (SPI-3), Magdalena")
...
def main() -> None:
    ...
    logger.info("=" * 70)
    logger.info("CLIMATE ANALYSIS FOR FLOOD CONTEXT - MAGDALENA, COLOMBIA")
    logger.info("=" * 70)
