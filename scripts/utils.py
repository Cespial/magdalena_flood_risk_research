...
def get_study_area() -> ee.FeatureCollection:
    """
    Return the Magdalena department boundary from FAO GAUL Level 1.

    Uses the ``FAO/GAUL/2015/level1`` dataset and filters for
    Magdalena in Colombia.  The result is a single-feature
    ``ee.FeatureCollection`` suitable for clipping and region operations.

    Returns
    -------
    ee.FeatureCollection
        Magdalena department polygon.
    """
    magdalena = (
...
def get_study_area_geometry() -> ee.Geometry:
    """
    Return the dissolved geometry of Magdalena as a single ``ee.Geometry``.
...
def get_municipalities() -> ee.FeatureCollection:
    """
    Return all municipalities within Magdalena from FAO GAUL Level 2.

    Returns
    -------
    ee.FeatureCollection
        ~30 municipality polygons with properties including ``ADM2_NAME``.
    """
    municipalities = (
...
def get_subregion_municipalities(subregion_name: str) -> ee.FeatureCollection:
    """
    Filter municipalities belonging to a specific Magdalena subregion.

    Parameters
    ----------
    subregion_name : str
        One of the 5 official Magdalena subregions defined in
        ``gee_config.SUBREGIONS`` (e.g. ``'Santa Marta'``).
...
def export_to_drive(
    ...
    folder: str = "magdalena_flood_risk",
    crs: str = "EPSG:4326",
    max_pixels: int = 1e13,
) -> ee.batch.Task:
    """
...
def get_dem() -> ee.Image:
    """
    Load the SRTM 30 m DEM clipped to Magdalena.
...
def _self_test() -> None:
    ...
    # Study area
    magdalena = get_study_area()
    n_features = safe_getinfo(magdalena.size(), label="magdalena.size()")
    _log.info("Magdalena feature count: %s", n_features)
    ...
    # Subregion filter
    sm = get_subregion_municipalities("Santa Marta")
    n_sm = safe_getinfo(sm.size(), label="santa_marta.size()")
    _log.info("Santa Marta municipalities: %s", n_sm)
